...
# --------------------------------------------------
# Brain volume columns — full parcellated set, csf excluded
# --------------------------------------------------
brain_cols = [
    ...
    "left accumbens area", "left ventral DC",
...
def fit_dag_bootstrap(
    ...
    report_every = max(1, n_bootstrap // 20)  # every 5%

    for b in range(n_bootstrap):

        if (b + 1) % report_every == 0 or b == 0:
            pct = 100 * (b + 1) / n_bootstrap

            prefix = ""
            if repeat_idx is not None:
                prefix += f"[Repeat {repeat_idx}] "
            if fold_idx is not None:
                prefix += f"[Fold {fold_idx + 1}/5] "

            print(
                f"{prefix}Bootstrap {b+1}/{n_bootstrap} ({pct:.1f}%)",
                file=sys.stderr,
                flush=True
            )
        ...
        try:
            ...
            print(
                f"{prefix}Bootstrap {b+1} failed: {e}",
                file=sys.stderr,
                flush=True
            )
            continue
...
# Load data
# --------------------------------------------------
df_t1 = pd.read_csv("/blue/neurology-dept/JOSH/_UF_DBS/T1_synthseg_vols_robust_parc.csv")
...
# Binary labels: GO=0 (score<3), NO-GO=1 (score>=3)
...
if repeat_idx == 0:
    ...
    print(f"Number of features: {len(brain_cols)}")
...
for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(df_features)):
    print(
        f"[Repeat {repeat_idx}] Starting outer fold {fold_idx + 1}/5",
        file=sys.stderr,
        flush=True
    )
...
os.makedirs("classification_parc_results", exist_ok=True)
out_df.to_csv(f"classification_parc_results/repeat_{repeat_idx:02d}.csv", index=False)
