...
def build_prior(n_features):
    ...
    for j in range(1, n_features):
        prior[AGE_IDX, j] = 0   # age -> brain: allowed
        prior[j, AGE_IDX] = 1   # brain -> age: forbidden
    return prior
...
os.makedirs("regression_age_dag_31_results", exist_ok=True)
out_df.to_csv(
    f"regression_age_dag_31_results/repeat_{repeat_idx:02d}.csv", index=False)
