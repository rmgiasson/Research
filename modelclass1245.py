...
# --------------------------------------------------
# Load data — exclude score == 3
# GO=0 (score<3), NO-GO=1 (score>3)
# --------------------------------------------------
...
# Exclude subjects with score exactly 3
df = df[df["neuropsych_score"] != 3].copy()
df["label"] = (df["neuropsych_score"] > 3).astype(int)

if repeat_idx == 0:
    print(f"Subjects after merge and exclusion of score==3: {len(df)}")
...
os.makedirs("classification_strict_31_results", exist_ok=True)
out_df.to_csv(
    f"classification_strict_31_results/repeat_{repeat_idx:02d}.csv",
    index=False)
