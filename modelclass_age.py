...
def preprocess_train_test(df_train, df_test):
    ...
    # Standardize covariates on train only
...
# --------------------------------------------------
# Bootstrap DAG — brain volumes only, no covariates
# --------------------------------------------------
...
# --------------------------------------------------
# Classifier — covariates appended to features
# --------------------------------------------------
param_grid = {
    "C":        [0.01, 0.1, 1.0, 10.0],
...
# --------------------------------------------------
# Load data — only keep subjects with age and education
# --------------------------------------------------
...
for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(df_brain)):
    ...
    # DAG fit on brain volumes only — covariates not included
    ...
    # --- Condition 1: original + covariates ---
    ...
    res.update({"repeat": repeat_idx, "fold": fold_idx,
                "condition": "original_cov"})
    fold_rows.append(res)

    # --- Condition 2: residual + covariates ---
    res = evaluate_classifier(
    ...
    res.update({"repeat": repeat_idx, "fold": fold_idx,
                "condition": "residual_cov"})
    fold_rows.append(res)

    # --- Condition 3: original + residual + covariates ---
    res = evaluate_classifier(
    ...
    res.update({"repeat": repeat_idx, "fold": fold_idx,
                "condition": "original_residual_cov"})
    fold_rows.append(res)

out_df = pd.DataFrame(fold_rows)
os.makedirs("classification_cov_31_results", exist_ok=True)
out_df.to_csv(
    f"classification_cov_31_results/repeat_{repeat_idx:02d}.csv", index=False)
